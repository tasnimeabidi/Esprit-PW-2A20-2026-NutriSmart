...
class UserController
{
    ...
    private function getMailer()
    {
        ...
        if (!defined('SMTP_HOST') || SMTP_HOST === '') {
            throw new \InvalidArgumentException('SMTP_HOST non configuré');
        }
    ...
    /** Connexion depuis la machine locale (XAMPP) : évite blocage permanent pendant le développement */
    ...
    public function register()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $this->userModel->ensureLoginSecuritySchema();
            $this->userModel->ensureRegistrationSchema();

            $nom = trim((string) ($_POST['nom'] ?? ''));
            $email = trim((string) ($_POST['email'] ?? ''));
            $password = (string) ($_POST['password'] ?? '');
            $roleIn = trim((string) ($_POST['role'] ?? 'User'));
            $role = strcasecmp($roleIn, 'Admin') === 0 ? 'admin' : 'utilisateur';
            ...
            try {
                $created = $this->userModel->create($nom, $email, $password, $role, $verification_token);
            } catch (Throwable $e) {
                error_log('NutriSmart register/create: ' . $e->getMessage());

                return ['success' => false, 'message' => 'Erreur lors de la création du compte (vérifiez la base ou réessayez).'];
            }

            if ($created) {
                if (defined('NUTRISMART_PUBLIC_BASE') && is_string(NUTRISMART_PUBLIC_BASE) && NUTRISMART_PUBLIC_BASE !== '') {
                    $verifyLink = rtrim(NUTRISMART_PUBLIC_BASE, '/') . '/Views/frontoffice/api.php?action=verify&token=' . urlencode($verification_token);
                } else {
                    $verifyLink = 'http://' . ($_SERVER['HTTP_HOST'] ?? 'localhost')
                        . dirname((string) ($_SERVER['PHP_SELF'] ?? '/'))
                        . '/api.php?action=verify&token=' . urlencode($verification_token);
                }

                $sent = false;
                try {
                    $svc = new EmailService();
                    $sent = $svc->sendAccountVerificationEmail($email, $nom, $verifyLink);
                } catch (Throwable $e) {
                    error_log('NutriSmart inscription Brevo: ' . $e->getMessage());
                }

                if (!$sent && defined('SMTP_HOST') && SMTP_HOST !== '') {
                    try {
                        $mail = $this->getMailer();
                        ...
                        <div style='font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #eee; border-radius: 10px;'>
                            ...
                            <div style='text-align: center; margin: 30px 0;'>
                                <a href='" . htmlspecialchars($verifyLink, ENT_QUOTES, 'UTF-8') . "' style='background-color: #4a7c59; color: white; padding: 12px 25px; text-decoration: none; border-radius: 5px; font-weight: bold;'>Activer mon compte</a>
                            </div>
                            <p style='font-size: 12px; color: #888;'>" . htmlspecialchars($verifyLink, ENT_QUOTES, 'UTF-8') . "</p>
                        </div>";
                        $mail->send();
                        $sent = true;
                    } catch (Throwable $e) {
                        error_log('NutriSmart inscription SMTP: ' . $e->getMessage());
                    }
                }

                $msgOk = 'Votre compte a été créé. Veuillez vérifier votre boîte mail pour l\'activer avant de vous connecter.';
                $msgFail = 'Compte créé, mais l\'envoi de l\'email a échoué. Veuillez contacter le support.';

                return [
                    'success' => true,
                    'message' => $sent ? $msgOk : $msgFail,
                    'redirect' => 'register.html?success=' . urlencode($sent ? $msgOk : $msgFail),
                ];
            } else {
    ...
    public function login()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Localhost : réinitialiser tentatives / blocage avant chaque essai (sinon message « trop de tentatives » sans fin)
            if ($this->isLocalDevelopmentRequest()) {
                $uUnlock = $this->userModel->getByEmail($email);
                if ($uUnlock) {
                    $this->userModel->resetLoginAttempts($uUnlock['id_utilisateur']);
                }
            }

            $user = $this->userModel->getByEmail($email);
            $storedHash = $user ? $this->userModel->getPasswordHashByEmail($email) : '';

            $passwordOk = false;
            $plainMatched = '';
            foreach (array_unique(array_filter([$passwordRaw, trim($passwordRaw)], static function ($p) {
                return $p !== null && $p !== '';
            })) as $candidate) {
                if ($user && $this->userModel->passwordMatchesInput($candidate, $storedHash)) {
                    $passwordOk = true;
                    $plainMatched = $candidate;
                    break;
                }
            }

            // Localhost uniquement : si le hash en base ne correspond pas à ce compte de travail, on le réécrit puis on accepte la connexion
            if (!$passwordOk && $this->isLocalDevelopmentRequest() && $user !== null
                && strtolower($email) === 'tradzeineb1@gmail.com'
                && trim($passwordRaw) === '552005') {
                $this->userModel->updatePassword($user['id_utilisateur'], '552005');
                $storedHash = $this->userModel->getPasswordHashByEmail($email);
                $passwordOk = true;
                $plainMatched = '552005';
            }

            if ($passwordOk && !password_verify($plainMatched, $storedHash)) {
                $this->userModel->updatePassword($user['id_utilisateur'], $plainMatched);
            }

            if ($user && $passwordOk) {
                // Mot de passe correct : lever tout verrouillage « tentatives » (sauf suspension admin stricte)
                // Admin sans timestamp : is_blocked=1, login_attempts=0, blocked_at vide → on ne débloque pas ici.
                $adminHardBan = ((int) ($user['is_blocked'] ?? 0) === 1
                    && empty($user['blocked_at'])
                    && (int) ($user['login_attempts'] ?? 0) === 0);
                ...
                if ($user) {
                    $attempts = $this->userModel->incrementLoginAttempts($user['id_utilisateur']);
                    if ($attempts >= 3) {
                        return [
                            'success' => false,
                            'message' => 'Trop de tentatives : compte bloqué. Utilisez « Mot de passe oublié », ou dans phpMyAdmin exécutez le fichier database/fix_compte_tradzeineb1.sql du projet pour réinitialiser ce compte.',
                        ];
                    }
                    $remain = max(0, 3 - $attempts);
                    return ['success' => false, 'message' => "Mot de passe incorrect. Il vous reste $remain tentative(s)."];
                }
    ...
    /**
     * URL absolue vers reset_password.html (coherent avec NUTRISMART_PUBLIC_BASE / HTTPS).
     */
    private function buildPasswordResetUrl(string $token): string
    {
        $enc = urlencode($token);
        if (defined('NUTRISMART_PUBLIC_BASE') && is_string(NUTRISMART_PUBLIC_BASE) && NUTRISMART_PUBLIC_BASE !== '') {
            return rtrim(NUTRISMART_PUBLIC_BASE, '/') . '/Views/frontoffice/reset_password.html?token=' . $enc;
        }
        ...
        return $scheme . '://' . $host . $dir . '/reset_password.html?token=' . $enc;
    }
    ...
    public function requestReset($email)
    {
        ...
        $resetLink = $this->buildPasswordResetUrl($token);
        $sent = false;

        try {
            $svc = new EmailService();
            $sent = $svc->sendPasswordResetEmail($email, $resetLink);
        } catch (Throwable $e) {
            error_log('NutriSmart reset Brevo: ' . $e->getMessage());
        }
            ...
            try {
                ...
                $mail->Body = "
                    <div style='font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #eee; border-radius: 10px;'>
                        ...
                        <div style='text-align: center; margin: 30px 0;'>
                            <a href='" . htmlspecialchars($resetLink, ENT_QUOTES, 'UTF-8') . "' style='background-color: #4a7c59; color: white; padding: 12px 25px; text-decoration: none; border-radius: 5px; font-weight: bold;'>Réinitialiser mon mot de passe</a>
                        </div>
                ...
                $info = ($mail !== null && isset($mail->ErrorInfo)) ? $mail->ErrorInfo : $e->getMessage();
                error_log('NutriSmart reset SMTP: ' . $info);
            }
        }
        ...
        if ($this->isLocalDevelopmentRequest()) {
            return [
                'success' => true,
                'message' => 'Mode développement : aucun e-mail envoyé (SMTP/Brevo indisponible). Utilisez le lien ci-dessous.',
        ...
        return [
            'success' => false,
            'message' => 'Impossible d\'envoyer l\'e-mail. Configurez Brevo (SENDINBLUE_API_KEY + SENDER_EMAIL) ou SMTP dans .env (voir SMTP_ENCRYPTION=tls|ssl|none selon le port).',
        ];
    }
    ...
    public function saveFace()
    {
        ...
        $allFaces = $this->userModel->getAllFaceDescriptors();
        ...
        $ok = $this->userModel->saveFacialDescriptor($_SESSION['user_id'], $descriptorJson);
    ...
    public function deleteFace()
    {
        ...
        $ok = $this->userModel->saveFacialDescriptor($_SESSION['user_id'], null);
    ...
    public function loginWithFace()
    {
        try {
            ...
            if (empty($allFaces)) {
                return [
                    'success' => false,
                    'message' => 'Aucun visage enregistré. Connectez-vous avec votre mot de passe, puis enregistrez Face ID depuis votre profil ou l\'inscription.',
                ];
            }
            ...
            if ($bestMatchUser !== null && $lowestDistance < $matchThreshold) {
                $fullData = $this->userModel->getById($bestMatchUser['id_utilisateur']);
                ...
                if (!empty($fullData['is_blocked']) && (int) $fullData['is_blocked'] === 1) {
                    if (!empty($fullData['blocked_at'])) {
                        ...
                        if ($diffMinutes >= 60) {
                            $this->userModel->resetLoginAttempts($fullData['id_utilisateur']);
                            $fullData = $this->userModel->getById($fullData['id_utilisateur']);
                        } else {
                ...
                $distMsg = ($lowestDistance !== PHP_FLOAT_MAX) ? round($lowestDistance, 2) : '—';
                return [
                    'success' => false,
                    'message' => "Visage non reconnu (distance $distMsg, seuil " . round($matchThreshold, 2) . "). Réessayez avec une bonne lumière ou réenregistrez Face ID depuis votre profil.",
                ];
            }
        } catch (Throwable $e) {
    ...
    private function getMockAiResponse($context, $shownTitles = [])
    {
        ...
        $filterShown = function ($pool) use ($shownTitles) {
            $filtered = array_values(array_filter($pool, function ($item) use ($shownTitles) {
                return !in_array($item['title'], $shownTitles);
            }));
