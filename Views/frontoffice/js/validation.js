document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll('form').forEach(form => {
    ...
    // Ne pas reset les formulaires de connexion : sinon email/mot de passe effacés ou mélangés avec l'autofill
    const fid = form.id || '';
    if (fid !== 'loginForm' && fid !== 'modalLoginForm' && fid !== 'registerForm') {
      form.reset();
    }
  });

  // Ne pas vider les champs login après délai : espaces/collage invisible → « mot de passe incorrect »
  setTimeout(() => {
    document.querySelectorAll('input').forEach(input => {
      const form = input.closest('form');
      const fid = form && form.id ? form.id : '';
      if (fid === 'loginForm' || fid === 'modalLoginForm' || fid === 'registerForm') return;
  ...
  // --- Validators ---
  const validators = {
    nom: (val) => /^[a-zA-ZÀ-ÿ0-9\s'.-]{2,80}$/.test(val.trim()),
    email: (val) =>
      typeof NutriSmartValidate !== 'undefined' && NutriSmartValidate.email
        ? NutriSmartValidate.email(val).ok
        : /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(val.trim()),
  ...
  const errorMessages = {
    nom: "Le nom doit contenir entre 2 et 80 caractères (lettres, chiffres, espaces, tirets).",
      ...
      fetch(action, {
        ...
        .then(async (res) => {
          ...
          try {
            ...
            console.error('Réponse non-JSON (statut ' + res.status + ') :', text.slice(0, 400));
        ...
        .then(data => {
          ...
          if (data.success) {
            if (formId === 'registerForm' && typeof successContainer !== 'undefined' && successContainer) {
              ...
              let href = data.redirect || 'nutrismart-home.html';
              if (formId === 'loginForm' || formId === 'modalLoginForm') {
                try {
                  const next = new URLSearchParams(window.location.search).get('next');
                  const allowed = ['budget-user.php', 'budget-summary.php', 'user-achat.php'];
                  const serverTo = String(data.redirect || '');
                  if (
                    next &&
                    allowed.indexOf(next) !== -1 &&
                    serverTo.indexOf('backoffice') === -1
                  ) {
                    href = next;
                  }
                } catch (e) {}
            ...
            if (msg.includes('tentatives') || msg.includes('suspendu')) {
              ...
              if (submitBtn) {
                ...
                submitBtn.textContent = 'Accès bloqué';
              }
            }
        ...
        .catch(err => {
          ...
          alert(err && err.message === 'invalid_json'
            ? "Le serveur n'a pas renvoyé une réponse valide (vérifiez api.php et les logs PHP)."
            : "Erreur réseau ou réponse invalide du serveur.");
        });
    }
  });
  ...
  document.querySelectorAll('form').forEach(form => {
    form.addEventListener('input', (e) => {
      ...
      if (submitBtn && (submitBtn.disabled || submitBtn.textContent === 'Accès bloqué' || submitBtn.style.opacity === '0.5')) {
        submitBtn.disabled = false;
