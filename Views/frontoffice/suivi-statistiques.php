...
<head>
    ...
    <script src="js/nutrismart-validation.js"></script>
    ...
    <style>
        ...
        .msg.typing { display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap; }
        .msg.typing .typing-label {
...
<body>
            ...
            <div class="calorie-viz">
                <div class="progress-ring">
                    <svg class="ring-svg" width="140" height="140">
                        ...
                        <circle id="cal-ring-fill" class="ring-fill" cx="70" cy="70" r="60" style="stroke-dashoffset: <?php echo 376.8 - (376.8 * ($progressPercent/100)); ?>;" />
                    </svg>
                    <div class="ring-inner-text">
                        <span class="ring-val" id="cal-net-display"><?php echo number_format((float) $netConsumed, 0, ',', ' '); ?></span>
                        <span class="ring-lbl">Nettes (kcal)</span>
                    </div>
                </div>
                <div style="text-align: center;">
                    <p id="cal-goal-display" style="font-size: 0.9rem; font-weight: 700; color: var(--forest);">Objectif: <?php echo (int) $goal; ?> kcal</p>
                    <p id="cal-reste-display" style="font-size: 0.75rem; color: var(--gray);">Reste: <?php echo max(0, (int) round($remaining)); ?> kcal</p>
                </div>
            </div>
            <!-- AI Health Radar -->
    ...
    <script>
        function toggleChat() {
            ...
            const willOpen = chat.style.display !== 'flex';
            chat.style.display = willOpen ? 'flex' : 'none';
            if (willOpen) {
                fetch('../../controllers/SuiviController.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' },
                    body: 'action=warm_chat',
                }).catch(function () {});
            }
        }
        ...
        async function sendMessage() {
            ...
            const sendBtn = document.querySelector('#aiChat .send-btn');
            const messages = document.getElementById('chatMessages');
            const text = input.value.trim();
            if(!text) return;
            var chatOk = NutriSmartValidate.chatMessage(text);
            if (!chatOk.ok) {
                alert(chatOk.msg);
                return;
            }
            ...
            // Indicateur : le modèle local peut prendre du temps (surtout sur CPU)
            const typing = document.createElement('div');
            typing.className = 'msg bot typing';
            typing.innerHTML = '<span class="typing-label">NutriBot réfléchit…</span><span class="typing-dots"><span></span><span></span><span></span></span>';
            ...
            try {
                ...
                try {
                    ...
                    errMsg.textContent = 'Réponse invalide du serveur. Vérifiez PHP / la console réseau.';
                ...
                if (data.status === 'error') {
                    bMsg.style.color = '#c0392b';
                    bMsg.textContent = data.answer || 'Une erreur est survenue.';
                } else if (data.answer && data.answer.includes("Insufficient Balance")) {
                ...
                errMsg.className = 'msg bot';
                errMsg.style.color = '#c0392b';
                errMsg.textContent = 'Impossible de joindre le serveur. Vérifiez votre connexion ou que XAMPP tourne.';
                messages.appendChild(errMsg);
                messages.scrollTop = messages.scrollHeight;
            } finally {
        ...
        const CAL_RING_LEN = 376.8;

        function refreshCaloriesCardFromStats(stats) {
            if (!stats || typeof stats !== 'object') return;
            var consumed = parseInt(String(stats.consumed || 0), 10) || 0;
            var burned = parseInt(String(stats.burned || 0), 10) || 0;
            var goal = parseInt(String(stats.dynamicGoal || 0), 10) || 2000;
            var net = Math.max(0, consumed - burned);
            var remaining = Math.max(0, goal - net);
            var pct = goal > 0 ? Math.min(100, (net / goal) * 100) : 0;

            var netEl = document.getElementById('cal-net-display');
            if (netEl) netEl.textContent = Math.round(net).toLocaleString('fr-FR');

            var ring = document.getElementById('cal-ring-fill');
            if (ring) ring.style.strokeDashoffset = String(CAL_RING_LEN - (CAL_RING_LEN * (pct / 100)));

            var goalEl = document.getElementById('cal-goal-display');
            if (goalEl) goalEl.textContent = 'Objectif: ' + Math.round(goal).toLocaleString('fr-FR') + ' kcal';

            var resteEl = document.getElementById('cal-reste-display');
            if (resteEl) resteEl.textContent = 'Reste: ' + Math.round(remaining).toLocaleString('fr-FR') + ' kcal';

            var coach = document.getElementById('aiCoach');
            if (coach && stats.advice) {
                coach.innerHTML =
                    '✨ <strong>Statut du jour :</strong> Vous avez consommé ' +
                    consumed +
                    ' kcal. ' +
                    stats.advice;
            }
        ...
        function refreshStatsFromServer() {
            fetch('../../controllers/SuiviController.php?action=stats')
                .then(function (r) {
                    return r.json();
                })
                .then(function (json) {
                    var payload = json && json.data !== undefined ? json.data : json;
                    refreshCaloriesCardFromStats(payload);
                })
                .catch(function (e) {
                    console.error(e);
                });
        ...
        function analyzeAI(type) {
            ...
            const rawInput = document.getElementById(type === 'meal' ? 'foodInput' : 'sportInput').value.trim();
            ...
            var descCheck = NutriSmartValidate.logDescription(rawInput, type === 'meal' ? 'meal' : 'activity');
            if (!descCheck.ok) {
                alert(descCheck.msg);
                return;
            }
        ...
        function saveLog(type) {
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    var resBox = document.getElementById(type === 'meal' ? 'mealResult' : 'sportResult');
                    if (resBox) resBox.style.display = 'none';
                    lastPrediction = { type: '', desc: '', qty: 0, cal: 0 };
        ...
        function promptWeight() {
            ...
            if (w == null || w === '') return;
            var wv = NutriSmartValidate.weightKg(w);
            if (!wv.ok) {
                alert(wv.msg);
                return;
            }
            ...
            formData.append('weight', String(wv.value));
            formData.append('description', wv.value + ' kg (Étalonnage manuel)');
            formData.append('date', new Date().toISOString().split('T')[0]);
        ...
        document.getElementById('editLogForm').addEventListener('submit', function(e) {
            ...
            const rawDesc = document.getElementById('editLogDesc').value;
            if (logType === 'meal' || logType === 'activity') {
                const v = NutriSmartValidate.logDescription(rawDesc, logType);
                if (!v.ok) {
                    alert(v.msg);
                    return;
                }
