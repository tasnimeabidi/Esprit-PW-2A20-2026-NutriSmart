...
switch ($action) {
    ...
    error_log('api.php: ' . $e->getMessage() . ' @ ' . $e->getFile() . ':' . $e->getLine());
    $ra = $_SERVER['REMOTE_ADDR'] ?? '';
    $isLocal = ($ra === '127.0.0.1' || $ra === '::1' || strncmp((string) $ra, '127.', 4) === 0 || strpos((string) $ra, '::ffff:127.') === 0);
    $baseMsg = 'Erreur serveur. Vérifiez que la base « nutrismart » est importée et que les tables utilisateur / profil existent.';
    if ($isLocal) {
        $baseMsg .= ' [dev] ' . $e->getMessage();
    }
    respondJson([
        'success' => false,
        'message' => $baseMsg,
