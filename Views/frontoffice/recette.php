...
// Ancienne base : calories_100g / protéines_100g… → clés attendues par la vue
foreach ($aliments as &$rowAl) {
    if (!isset($rowAl['calories']) && isset($rowAl['calories_100g'])) {
        $rowAl['calories'] = $rowAl['calories_100g'];
    }
    if (!isset($rowAl['proteines']) && isset($rowAl['proteines_100g'])) {
        $rowAl['proteines'] = $rowAl['proteines_100g'];
    }
    if (!isset($rowAl['glucides']) && isset($rowAl['glucides_100g'])) {
        $rowAl['glucides'] = $rowAl['glucides_100g'];
    }
    if (!isset($rowAl['lipides']) && isset($rowAl['lipides_100g'])) {
        $rowAl['lipides'] = $rowAl['lipides_100g'];
    }
}
unset($rowAl);
try {
    ...
    $recettes_db = (new Recette($db))->readAll();
}
...
function sendMessage() {
  ...
  if (!message) return;
  var chk = NutriSmartValidate.chatMessage(message);
  if (!chk.ok) {
    alert(chk.msg);
    return;
  }
  ...
  .then(function (data) {
    if (data.success) {
      ...
      addMessageToChat(data.error || 'Envoi impossible.', false);
    }
  })
  .catch(function () {
    addMessageToChat(message, true);
    addMessageToChat('Connexion interrompue. Vérifiez que le serveur répond et réessayez.', false);
  });
}
  ...
  .then(function (data) {
    ...
    if (data.success && data.messages && data.messages.length > 0) {
      data.messages.forEach(function (msg) {
        var isUser = !msg.is_admin;
        addMessageToChat(msg.message, isUser);
      });
    } else {
