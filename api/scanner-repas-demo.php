...
try {
    ...
    $m = $e->getMessage();
    if (str_starts_with($m, 'GROQ_AUTH_FAILED:')) {
        $detail = trim(substr($m, strlen('GROQ_AUTH_FAILED:')));
        JsonApi::erreur(
            401,
            $detail !== ''
                ? 'Clé Groq refusée (401). ' . $detail
                : 'Clé Groq refusée ou absente (401). Vérifiez config/groq.local.php ou la variable GROQ_API_KEY.'
        );
        return;
    }
    JsonApi::erreur(502, $m);
}
